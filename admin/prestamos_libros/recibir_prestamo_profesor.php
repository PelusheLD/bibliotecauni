...
$fecha1 = $_POST["fecha1"];
$fecha2 = $_POST["fecha2"];
$profesor = $_POST["estudiante"];
$estado = "0";

$sql_estado_profesor = "SELECT status FROM usuario_profesores WHERE id_usuario_profesor = '$profesor'";
$resultado_estado_profesor = mysqli_query($con, $sql_estado_profesor);

if (mysqli_num_rows($resultado_estado_profesor) > 0) {
	$row_estado_profesor = mysqli_fetch_assoc($resultado_estado_profesor);
	$status_profesor = $row_estado_profesor['status'];

	if ($status_profesor == 'OK') {
		...
		if (mysqli_num_rows($resultado_libro) > 0) {
			...
			if ($ejemplares_disponibles > 0) {
				...
				$sql_insertar_prestamo = "INSERT INTO prestamo_libro_profesores (fecha_prestamo, fecha_entrega, id_libro, id_usuario_profesor, estado) 
            VALUES ('$fecha1', '$fecha2', '$id', '$profesor', '1')";
				...
				if ($resultado_insertar_prestamo) {
					...
					$sql_insertar_detalles = "INSERT INTO detalles_prestamo_profesores (id_usuario_profesor, carnet, nombre, fecha_prestamo, fecha_entrega, id_libro, contador) 
				VALUES ('$profesor', (SELECT carnet FROM usuario_profesores WHERE id_usuario_profesor = '$profesor'), (SELECT nombre FROM usuario_profesores WHERE id_usuario_profesor = '$profesor'), '$fecha1', '$fecha2', '$id', '1')";
	...
	} elseif ($status_profesor == 'Sancionado') {
		header('Location:' . $URL . '../prestar_libro.php');
		session_start();
		$_SESSION['error'] = "No se puede realizar el préstamo. El profesor está sancionado.";
	} elseif ($status_profesor == 'Vetado') {
		header('Location:' . $URL . '../prestar_libro.php');
		session_start();
		$_SESSION['error'] = "No se puede realizar el préstamo. El profesor está vetado.";
	} else {
		header('Location:' . $URL . '../prestar_libro.php');
		session_start();
		$_SESSION['error'] = "El estado del profesor es inválido.";
	}
} else {
	header('Location:' . $URL . '../prestar_libro.php');
	session_start();
	$_SESSION['error'] = "El profesor no existe.";
}
